...
function makeSpec(): ServiceSpec {
  return {
    ...
    dshBin: '/usr/local/lib/node_modules/@deepseek-ai/dsh/dist/cli.js',
    commandPath: '/usr/local/bin/node',
    commandArgs: ['/usr/local/lib/node_modules/@deepseek-ai/dsh/dist/cli.js', '--profile', 'default'],
...
describe('buildLaunchdPlist', () => {
  it('renders a LaunchAgent with KeepAlive without embedding private environment', () => {
    ...
    expect(plist).not.toContain('sk-secret');
    expect(plist).toContain('<key>StandardOutPath</key>');
    expect(plist).toContain('@deepseek-ai/dsh/dist/cli.js');
  ...
  it('escapes XML special characters', () => {
    ...
    expect(plist).not.toContain('/x &amp; y');
  });
});
...
describe('LaunchdServiceController', () => {
  ...
  it('stops by booting out the agent while keeping the plist installed', async () => {
    const agentDir = await mkdtemp(join(tmpdir(), 'dsh-lark-agents-'));
    await writeFile(
      join(agentDir, 'io.dsh-lark-bot.plist'),
      buildLaunchdPlist(makeSpec()),
