...
describe('buildSystemdUnit', () => {
  it('renders a user service with autostart and restart-on-failure', () => {
    ...
    expect(unit).toContain(
      'ExecStart=/usr/bin/node /home/user/lib/node_modules/dsh-lark-bot/dist/cli.js run --profile default',
    );
  ...
  it('quotes executable arguments containing spaces', () => {
    const unit = buildSystemdUnit(
      makeSpec({
        ...
        cliJsPath: '/home/user/my packages/dist/cli.js',
        profile: 'work bot',
      }),
    );
    expect(unit).toContain(
      'ExecStart="/opt/node 22/bin/node" "/home/user/my packages/dist/cli.js" run --profile "work bot"',
    );
  });
});
...
describe('SystemdServiceController', () => {
  ...
  it('disables autostart on stop', async () => {
    ...
    expect(calls.map((call) => call.args.join(' '))).toContain(
      '--user disable --now dsh-lark-bot.service',
    );
  });
});
