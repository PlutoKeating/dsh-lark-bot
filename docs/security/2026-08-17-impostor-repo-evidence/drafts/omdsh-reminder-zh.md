...
`[Security] 同生态开发者提醒：贡献者统计 ≠ 仓库归属，谨防“去 fork 化”假冒重传（附可复用取证经验）`

# 正文

## 关于我

我是 **PlutoKeating**，`dsh-lark-bot`（飞书 / Lark × DeepSeek Harness 桥接）的维护者。
`dsh-lark` 与 `dsh-lark-bot` 是同生态竞品，但我非常尊重 dsh-lark 作者的真实开发付出——
发这个帖子不是为了竞争，而是共享我们刚踩过的坑和一套可复用的防御方法。
