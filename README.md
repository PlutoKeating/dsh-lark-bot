...
> **全网最强（飞书单一通道）**：在 DSH「通道通知 × 飞书」生态里，我们**飞书功能最全、维护最勤**——
> 功能完备度与维护积极性两项均获满分（5.0/5.0），Issue 关闭率 **96% 全场第一**，v0.19.6 高频迭代、当日仍在更新
> （54k 行 / 325 文件，配 `ARCHITECTURE.md` + `API.md`）。我们不追多平台广度（微信 / 钉钉 / QQ 等另算），
> 而是**把飞书这一个场景做到最深、最省心、最耐用**。

- **安全网守护**（**全网唯一**）：dsh 崩溃后飞书仍会回复你，`/safemode` 进入仅核心安全模式直接自愈——多数桥接方案是「串行单聊 + 崩溃就失联」。
...
- **崩溃可对账**：消息先写持久任务账本再入队，重启恢复排队项，`/jobs` 显式重试。
- **会话归档与保留**：`/archive` + `/retention` 自动归档与清理，会话列表不会烂掉。
- **跨会话通知 + @人**：A 群跑完任务主动推送到 B 群 / 私聊并 @ 你。
- **对话内管理模型与密钥**：一张 `/config` 卡片切换供应商、热更新密钥，不用离开飞书。
- **快速 / 平衡 / 深度模式**：`/mode` 一键选任务强度，下一轮生效且不打断当前任务。
- **关键任务先拍板**：`lark_request_plan_approval` 先出完整计划，再批准或附意见继续规划，原任务自动续跑。
- **dsh Web 可视化设置**：官方 Settings → Plugins 页面点选工作目录、模型、并行数、提醒，不用背环境变量。
- **飞书内自更新**：管理员 `/upgrade` 后台更新、验证并重载；有新版本才提醒。
