...
| `/replies [show\|default\|set …]` | 配置当前 scope 的回复合并、发送间隔、批量上限与近似去重（修改仅管理员）|
...
**回复流量控制**：默认保持即时逐条回复。管理员可用 `/replies set merge=5 batch=3 interval=10 dedupe=60` 为当前 scope 开启 5 秒合并窗口、每条合并最多 3 个任务、两批至少间隔 10 秒，并在 60 秒内抑制同一发送者在同 workspace 的近似重复任务；超出批量上限的答案继续排队，不会丢弃。`/replies` 与 `/status` 显示有效策略，`/replies default` 恢复默认。
