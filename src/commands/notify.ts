...
async function handleNotifySend(args: string, ctx: CommandContext): Promise<void> {
  if (!ctx.accessManager.isAdmin(ctx.senderId)) {
    await reply(ctx, '仅管理员可执行该操作。');
    return;
  }
  const match = args.trim().match(/^(\S+)\s+([\s\S]+)$/);
  if (!match) {
    await reply(ctx, '用法：`/notify <scope|chatId> <text>`');
    return;
  }
  ...
  await reply(ctx, `已发送通知到 \`${target}\`（chat \`${destination.chatId}\`）。`);
}
...
async function handleNotifyList(_args: string, ctx: CommandContext): Promise<void> {
  ...
  if (scopes.length === 0) {
    await reply(ctx, '暂无已注册的 scope（至少需要一个会话发过消息）。');
    return;
  }
