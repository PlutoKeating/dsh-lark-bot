...
export type WindowsTaskAction = 'install' | 'stop' | 'restart' | 'status';
...
export function buildScheduledTaskScript(
  ...
  const vars = [
    ...
    `$node = ${psQuote(spec.nodePath)}`,
    `$cli = ${psQuote(spec.cliJsPath)}`,
    `$log = ${psQuote(spec.logFile)}`,
    `$profile = ${psQuote(spec.profile)}`,
    `$workDir = ${psQuote(dirname(spec.cliJsPath))}`,
  ].join('\n');

  const install = [
    `$inner = '"' + $node + '" "' + $cli + '" run --profile "' + $profile + '" >> "' + $log + '" 2>&1'`,
